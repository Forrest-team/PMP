...
class ValidCodeImg:
    def __init__(self, width=93, height=31, code_count=4, font_size=28,
                 point_count=20, line_count=4, img_format='png'):
    ...
    def get_valid_code_img(self):
        # 获取一个Image对象，参数分别是RGB模式。宽93，高31，随机颜色
        ...
        for i in range(self.code_count):
            ...
            draw.text((5 + i * 22, -1), random_char, self.get_random_color(), font=font)

            # 保存随机字符，以供验证用户输入的验证码是否正确时使用
            temp.append(random_char)
        valid_string = "".join(temp)
        ...
        # 在内存生成图片
        bto = BytesIO()
        image.save(bto, self.img_format)
        pic_data = bto.getvalue()
        bto.close()
